...
from tronapi.base.toolz import assoc
...
class Trx(Module):
    ...
    def get_block(self, block: Any = None):
        ...
        response = self.tron.manager.request(method["url"], {method["field"]: block})
        return response
    ...
    def get_account(self, address=None):
        ...
        data = self.tron.manager.request(
            "/wallet/getaccount", {"address": self.tron.address.to_hex(address)}
        )

        if data.get("account_name") is not None:
            data["account_name"] = binascii.unhexlify(data["account_name"]).decode()
        return data
    ...
    def get_transactions_related(self, address, direction="all", limit=30, offset=0):
        ...
        if direction == "all":
            from_direction = {
                "from": self.get_transactions_related(address, "from", limit, offset)
            }
            to_direction = {
                "to": self.get_transactions_related(address, "to", limit, offset)
            }
    ...
    def get_transaction_info(self, tx_id):
        ...
        if response.get("internal_transactions") is not None:
            for inner_tx in response["internal_transactions"]:
                inner_tx["note"] = binascii.unhexlify(inner_tx["note"]).decode()
    ...
    def send_transaction(self, to, amount, options=None):
        ...
        Args:
            ...
            amount (int/float): Amount of TRX to send.
    ...
    def sign(self, transaction: Any, use_tron: bool = True):
        ...
        signed_tx = Account.signHash(
            message_hash=transaction["txID"], private_key=self.tron.private_key
        )
    ...
    def verify_message(
        ...
        recovered = Account.recoverHash(
            message_hash, signature=signed_message.signature
        )
    ...
    def contract(self, address=None, **kwargs):
        contract_factory_class = kwargs.pop(
